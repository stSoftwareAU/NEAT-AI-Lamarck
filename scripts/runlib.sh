#!/usr/bin/env bash
# Build, install, and clean neat_ai_lamarck for the fleet worker (#236).
#
# Installs the crate CLI only:
#   ~/.cargo/bin/neat_ai_lamarck
# stamped with .neat_ai_lamarck.version beside it.
#
# stdout is the CLI binary path only. Diagnostics go to stderr.
# A second run whose stamp matches prints
#   [neat_ai_lamarck] already installed v<x>
...
# NEAT-AI-core#680 / this repo #234. This ships the install contract so the
# fleet can run Lamarck stages.
#
