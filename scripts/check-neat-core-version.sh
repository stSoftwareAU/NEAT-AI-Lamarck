...
#
# Lamarck consumes neat-core through an UNPINNED `path` dependency that always
# tracks head (see `lamarck/Cargo.toml`). The safeguard is this CI gate: it
# fails when the sibling neat-core presents a breaking bump Lamarck has not yet
# acknowledged, forcing a deliberate upgrade instead of tracking head blindly.
#
# Mechanism — version-baseline check:
#   * Lamarck records the last-handled neat-core version in the checked-in
...
#
# "Handling" a breaking bump = a deliberate Lamarck PR that makes the
...
remediation() {
  cat >&2 <<EOF
       neat-core has presented a breaking bump Lamarck has not handled.
       To clear this gate, in a single deliberate PR:
         1. Update lamarck for the breaking neat-core change.
